...
import { check, param, body,validationResult, sanitizeParam} from 'express-validator'
...
ProjectRouter.post(
    ...
    .notEmpty()
    .isLength({ min: 3 })
    .withMessage("name longer than 3 characters"),generalValidate, createProject)

ProjectRouter.put("/:project_id([0-9]+)",
    check("name")
    .notEmpty()
    .isLength({ min: 3 })
...
TasklistRouter.post("/",
param('project_id').isNumeric().withMessage("project_id must be numeric"),
 generalValidate, createTasklist);

TasklistRouter.put("/:tasklist_id([0-9]+)",
param('project_id').isNumeric().withMessage("project_id must be numeric"),
param('tasklist_id').isNumeric().withMessage("tasklist_id must be numeric"),
generalValidate,
updateTasklist)
TasklistRouter.delete("/:tasklist_id([0-9]+)",
param('project_id').isNumeric().withMessage("project_id must be numeric"),
param('tasklist_id').isNumeric().withMessage("tasklist_id must be numeric"),
generalValidate
,deleteTasklist)
